...
public class EntityRedAnt extends EntityNewAnimal
    implements INewMob
{
    ...
    public boolean getCanSpawnHere()
    {
        ...
        return worldObj.checkIfAABBIsClear(boundingBox) && worldObj.getCollidingBoundingBoxes(this, boundingBox).size() == 0 && !worldObj.isAnyLiquid(boundingBox) && (worldObj.getBlockId(i, j - 1, k) == Block.stone.blockID || worldObj.getBlockId(i, j - 1, k) == Block.sand.blockID);
    }
    ...
    {
		Entity nn = null;
		if(mod_SpiderQueen.AntLike > 24)
		{
			nn = worldObj.getClosestPlayerToEntity(this, 16D); 
			if(nn != null) { return nn; }
		}
        return mod_SpiderQueen.getClosestEntityToEntity(worldObj, this, 16D, 3);
    }
    ...
    public void onLivingUpdate()
    {
		...
		if(tracking != null) { EntityRedAnt nne = (EntityRedAnt)tracking; if(nne.getTracking() == this) { tracking = null; } }
			...
			if(munch>20)
			{
				...
				for(int xx=-1; xx<=1; xx++){ for(int yy=-1; yy<=1; yy++){ for(int zz=-1; zz<=1; zz++){
					if(yy < 1 || rr.nextInt(5) == 0)
					{
							...
							{
								if(rr.nextInt(10) == 0 & (rr.nextInt(4) == 0 || xx != 0 || zz != 0 || yy < 1)) { worldObj.setBlockWithNotify(pX+xx,pY+yy,pZ+zz,0); }
							}
					}
				}}}
			}			
		...
		else
		{
			if(entityToAttack == null)
			{
				...
				if(tracking == null)
				{
					...
					if(rm2.nextInt(5) == 2)
					{
						tracking = mod_SpiderQueen.getClosestEntityToEntity(worldObj, this, 32D, 8);
