...
public class TileEntityAntHill extends TileEntity
{
	...
	public void updateEntity()
	{
		...
		// if(!worldObj.getChunkProvider().chunkExists(xCoord / 16,zCoord / 16))
		// { updateDelay(); return; }
		...
		for (int i = 0; i < byte0; i++)
		{
			...
			if (this.antType == 1)
			{
				entityliving = new EntityAnt(this.worldObj);
			}// EntityList.createEntityInWorld(mobID, worldObj);
			if (this.antType == 2)
			{
				entityliving = new EntityRedAnt(this.worldObj);
			}// EntityList.createEntityInWorld(mobID, worldObj);
			...
			if (this.antType == 1)
			{
				j = this.worldObj.getEntitiesWithinAABB(EntityAnt.class, AxisAlignedBB.getBoundingBox(this.xCoord, this.yCoord, this.zCoord, this.xCoord + 1, this.yCoord + 1, this.zCoord + 1).expand(32D, 32D, 32D)).size();
			}
			if (this.antType == 2)
			{
				j = this.worldObj.getEntitiesWithinAABB(EntityRedAnt.class, AxisAlignedBB.getBoundingBox(this.xCoord, this.yCoord, this.zCoord, this.xCoord + 1, this.yCoord + 1, this.zCoord + 1).expand(32D, 32D, 32D)).size();
			}
