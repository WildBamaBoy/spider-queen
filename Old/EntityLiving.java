...
public abstract class EntityLiving extends Entity
{
	...
	public void spiderBaitCode()
	{
		...
		if(changee == false)
		{
			...
			else
			{
				...
				if(tetarget != null)
				{
					int bbid = worldObj.getBlockId(tetarget.xCoord, tetarget.yCoord, tetarget.zCoord);
					if(bbid != mod_SpiderQueen.spiderbait.blockID) { tetarget = null; }
				}
    ...
    public boolean isOnLadder()
    {
		...
		{
		  if(worldObj.getBlockId(i-1, j + 1, k) == mod_SpiderQueen.smallWeb.blockID) { return true; }
		  if(worldObj.getBlockId(i+1, j + 1, k) == mod_SpiderQueen.smallWeb.blockID) { return true; }
		  if(worldObj.getBlockId(i, j + 1, k-1) == mod_SpiderQueen.smallWeb.blockID) { return true; }
		  if(worldObj.getBlockId(i, j + 1, k+1) == mod_SpiderQueen.smallWeb.blockID) { return true; }

		  if(worldObj.getBlockId(i-1, j + 0, k) == mod_SpiderQueen.smallWeb.blockID) { return true; }
		  if(worldObj.getBlockId(i+1, j + 0, k) == mod_SpiderQueen.smallWeb.blockID) { return true; }
		  if(worldObj.getBlockId(i, j + 0, k-1) == mod_SpiderQueen.smallWeb.blockID) { return true; }
		  if(worldObj.getBlockId(i, j + 0, k+1) == mod_SpiderQueen.smallWeb.blockID) { return true; }
		  
		  if(worldObj.getBlockId(i-1, j + 1, k) == Block.web.blockID) { return true; }
		  if(worldObj.getBlockId(i+1, j + 1, k) == Block.web.blockID) { return true; }
		  if(worldObj.getBlockId(i, j + 1, k-1) == Block.web.blockID) { return true; }
		  if(worldObj.getBlockId(i, j + 1, k+1) == Block.web.blockID) { return true; }

		  if(worldObj.getBlockId(i-1, j + 0, k) == Block.web.blockID) { return true; }
		  if(worldObj.getBlockId(i+1, j + 0, k) == Block.web.blockID) { return true; }
		  if(worldObj.getBlockId(i, j + 0, k-1) == Block.web.blockID) { return true; }
		  if(worldObj.getBlockId(i, j + 0, k+1) == Block.web.blockID) { return true; }
		  
		}
		
        return (worldObj.getBlockId(i, j, k) == Block.ladder.blockID || worldObj.getBlockId(i, j, k) == Block.vine.blockID || worldObj.getBlockId(i, j, k) == mod_SpiderQueen.smallWeb.blockID  || worldObj.getBlockId(i, j + 1, k) == mod_SpiderQueen.smallWeb.blockID || worldObj.getBlockId(i, j, k) == Block.web.blockID  || worldObj.getBlockId(i, j + 1, k) == Block.web.blockID);
    }
	// -SNIP
	...
	public boolean isOnWeb()
    {
        ...
        int j = MathHelper.floor_double(boundingBox.minY);
        int k = MathHelper.floor_double(posZ);
		if((worldObj.getBlockId(i, j + 1, k) == mod_SpiderQueen.smallWeb.blockID) || (worldObj.getBlockId(i, j + 1, k) == Block.web.blockID)) 
		{ 
