...
public class ItemCocoonWarrior extends Item
{

    public ItemCocoonWarrior(int i)
    {
        super(i);
        maxStackSize = 1;
    }
    ...
    {
		if(entityplayer.inventory.consumeInventoryItem(mod_SpiderQueen.itemCocoonWarrior.shiftedIndex))
        {
            //world.playSoundAtEntity(entityplayer, "random.bow", 1.0F, 1.0F / (itemRand.nextFloat() * 0.4F + 0.8F));
            if(!world.isRemote)
            {
			
					EntityCocoonWarrior entCocoon = new EntityCocoonWarrior(world, entityplayer.posX, entityplayer.posY, entityplayer.posZ, entityplayer.rotationYaw - 90F,itemstack.getItemDamage());
