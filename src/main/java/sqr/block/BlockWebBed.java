...
public class BlockWebBed extends Block
{
	...
	{
		boolean die = false;
		if(world.getBlock(x-1,y,z) == Blocks.air) { die = true; }
		if(world.getBlock(x+1,y,z) == Blocks.air) { die = true; }
		if(world.getBlock(x,y,z-1) == Blocks.air) { die = true; }
		if(world.getBlock(x,y,z+1) == Blocks.air) { die = true; }

		if (die)
		{
			this.dropBlockAsItem(world, x, y, z, Block.getIdFromBlock(b), 1); //TODO may not work.
...
//		{
//			entityplayer.addChatMessage(new ChatComponentText("tile.bed.noSleep"));
//		}
//		
		return true;
	}
