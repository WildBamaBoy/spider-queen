...
public class TileEntityBeeHive extends TileEntity
{
	...
	public void updateEntity()
	{
		...
		if (this.worldObj.getBlock(this.xCoord, this.yCoord + 1, this.zCoord) == ModBlocks.beehive)
		{
			return;
		}
		if (this.worldObj.getBlock(this.xCoord + 1, this.yCoord + 1, this.zCoord) == ModBlocks.beehive)
		{
			return;
		}
		if (this.worldObj.getBlock(this.xCoord - 1, this.yCoord + 1, this.zCoord) == ModBlocks.beehive)
		...
		if (this.worldObj.getBlock(this.xCoord, this.yCoord + 1, this.zCoord + 1) == ModBlocks.beehive)
		{
			return;
		}
		if (this.worldObj.getBlock(this.xCoord, this.yCoord + 1, this.zCoord - 1) == ModBlocks.beehive)
		...
		{
			final EntityLivingBase entityliving = new EntityGatherer(this.worldObj);// EntityList.createEntityInWorld(mobID,
																					// worldObj);
			...
			final EntityLivingBase entityliving2 = new EntityWarrior(this.worldObj);// EntityList.createEntityInWorld(mobID,
																					// worldObj);
			...
			if (!noWarrior)
			{
				...
				if (this.worldObj.isAirBlock((int) d6, (int) d7, (int) d8) && this.worldObj.isAirBlock((int) d6 - 1, (int) d7, (int) d8) && this.worldObj.isAirBlock((int) d6 + 1, (int) d7, (int) d8) && this.worldObj.isAirBlock((int) d6 - 2, (int) d7, (int) d8) && this.worldObj.isAirBlock((int) d6 + 2, (int) d7, (int) d8) && this.worldObj.isAirBlock((int) d6, (int) d7 - 1, (int) d8) && this.worldObj.isAirBlock((int) d6, (int) d7 + 1, (int) d8) && this.worldObj.isAirBlock((int) d6, (int) d7 - 2, (int) d8) && this.worldObj.isAirBlock((int) d6, (int) d7 + 2, (int) d8) && this.worldObj.isAirBlock((int) d6, (int) d7, (int) d8 - 1) && this.worldObj.isAirBlock((int) d6, (int) d7, (int) d8 + 1) && this.worldObj.isAirBlock((int) d6, (int) d7, (int) d8 - 2) && this.worldObj.isAirBlock((int) d6, (int) d7, (int) d8 + 2))
				{
			...
			// {
			// final EntityLivingBase entityliving3 = new
			// EntityQueenBee(this.worldObj);//EntityList.createEntityInWorld(mobID,
			// worldObj);
			...
			// }
			// final int j3 =
			// this.worldObj.getEntitiesWithinAABB(entityliving3.getClass(),
			// AxisAlignedBB.getBoundingBox(this.xCoord, this.yCoord,
			// this.zCoord, this.xCoord + 1, this.yCoord + 1, this.zCoord +
			// 1).expand(64D, 32D, 64D)).size();
			// if(j3 != 0)
			// {
			// this.updateDelay();
			// }
			...
			// {
			//
			// final double d6 = this.xCoord + (this.worldObj.rand.nextDouble()
			// - this.worldObj.rand.nextDouble()) * 16D;
			// final double d7 = this.yCoord + this.worldObj.rand.nextInt(3) -
			// 1;
			// final double d8 = this.zCoord + (this.worldObj.rand.nextDouble()
			// - this.worldObj.rand.nextDouble()) * 16D;
			//
			...
			// this.worldObj.isAirBlock((int)d6,(int)d7,(int)d8+2) )
			// {
			// entityliving3.setLocationAndAngles(d6, d7, d8,
		...
		// if(mod_SpiderQueen.hiveAlert) { mod_SpiderQueen.hiveAlert = false;
		// mod_SpiderQueen.pissOffBees(this.worldObj, null, this.xCoord,
		// this.yCoord, this.zCoord, 64D); }
		super.updateEntity();
	}
