...
public class WorldGenHugeTree extends WorldGenerator
{
        ...
        for(; j1 <= i1; j1++)
        {
            ...
            for(int l1 = -i1; l1 <= i1;)
            {
                ...
                } else
                {
                    ...
                    Block b = worldObj.getBlock(ai1[0], ai1[1], ai1[2]);
                    if(b != Blocks.air && b != Blocks.leaves && b != Blocks.leaves2)
                    {
                        l1++;
                    } else
                    {
                        worldObj.setBlock(ai1[0], ai1[1], ai1[2], Blocks.air);
                        l1++;
                    }
                }
            }
    ...
    void func_522_a(int ai[], int ai1[], int i)
    {
        ...
        for(int l = ai2[j] + byte3; k != l; k += byte3)
        {
            ...
            worldObj.setBlock(ai3[0], ai3[1], ai3[2], Blocks.air);
        }
	...
	{
		worldObj.setBlock(i, j-1, k, ModBlocks.beehive);
		worldObj.setBlock(i, j-2, k, ModBlocks.beehive);
		worldObj.setBlock(i, j-3, k, ModBlocks.beehive);
		worldObj.setBlock(i, j-4, k, ModBlocks.beehive);
		worldObj.setBlock(i, j-5, k, ModBlocks.beehive);
		worldObj.setBlock(i, j-6, k, ModBlocks.beehive);
		...
		for(int x=-7; x<7; x++)
		{
			for(int y=-7; y<7; y++)
			{
				for(int z=-7; z<7; z++)
				{
					...
					{
						worldObj.setBlock(i+x,j+y,k+z, ModBlocks.beehive);
					}
				}
			}
		}
		...
		if(oi > i)
		{
			for(int x=i; x<oi; x++)
			{
				worldObj.setBlock(x, j, k, Blocks.log);
			}
			return;
		}
		if(i > oi)
		...
		if(ok > k)
		{
			for(int z=k; z<ok; z++)
			{
				worldObj.setBlock(i, j, z, Blocks.log);
			}
			return;
		}
		if(k > ok)
	...
	{
		if (worldObj.getBlock(i,j,k) == Blocks.leaves || worldObj.getBlock(i,j,k) == Blocks.log)
		{
			if(worldObj.isAirBlock(i,j-1,k) 
					&& worldObj.isAirBlock(i,j-2,k) 
					&& worldObj.isAirBlock(i,j-3,k) 
					&& worldObj.isAirBlock(i,j-4,k) 
					&& worldObj.isAirBlock(i,j-5,k) 
					&& worldObj.isAirBlock(i,j-6,k) 
					&& worldObj.isAirBlock(i,j-7,k) 
					&& worldObj.isAirBlock(i,j-8,k))
			{
				return true;
			}
		}
        ...
        for (int j1 = j; j1 <= j + 1 + i; j1++)
        {
            ...
            for (int i1 = i - byte0; i1 <= i + byte0 && flag; i1++)
            {
                for (int k1 = k - byte0; k1 <= k + byte0 && flag; k1++)
                {
                    if (j1 >= 0 && j1 < 256)
                    {
                        Block b = world.getBlock(i1, j1, k1);
