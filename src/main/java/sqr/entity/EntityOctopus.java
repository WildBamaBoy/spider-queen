...
public class EntityOctopus extends EntityCreature implements INewMob
{
	...
	protected void attackEntity(Entity entity, float f)
	{
		...
		if (this.natk > 0)
		{
			...
			{
				if (this.posY + 0.5F - 0.3F > entity.posY)
				{
					this.motionY = this.motionY - 0.25F;
				}
				if (this.posY + 0.5F + 0.3F < entity.posY)
				{
					this.motionY = this.motionY + 0.25F;
		...
		if (this.natk == 0)
		{
			if (f > 18.0F & this.entityToAttack == entity)
			{
	...
	public void onLivingUpdate()
	{
		...
		if (this.worldObj.isAirBlock((int) this.posX, (int) (this.posY + 1.17F), (int) this.posZ))
		{
			dd = 0;
		}
		...
		if (this.entityToAttack != null)
		{
			...
			if (f1 > 13)
			{
				this.motionY = this.motionY - 0.05F;
			}
		}
		...
		// {
		// if(!this.worldObj.isAnyLiquid(this.boundingBox) ||
		// this.worldObj.getBlock((int)this.posX,(int)(this.posY +
		// 0.2F),(int)this.posZ) == 0) { this.motionY = this.motionY * 0.9F -
		// 0.1F; }
		// else
		// {
		...
		//
		//
		// if(this.worldObj.getBlock((int)this.posX,(int)(this.posY +
		// 1.17F),(int)this.posZ) == 0)
		// {
		// this.motionY = -0.2F;
		// }
		// else
