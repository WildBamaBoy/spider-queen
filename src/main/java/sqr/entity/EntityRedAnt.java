...
public class EntityRedAnt extends EntityCreature implements INewMob
{
	...
	public void onLivingUpdate()
	{
		...
		if (this.tracking != null)
		{
			final EntityRedAnt nne = (EntityRedAnt) this.tracking;
			if (nne.getTracking() == this)
			{
				this.tracking = null;
			}
		}
		...
		if (this.isCollidedHorizontally & this.isCollidedVertically)
		{
			...
			if (this.munch > 20)
			{
				...
				// if(id == Blocks.dirt || id == Blocks.grass || id ==
				// Blocks.stone)
				// {
				// if(rr.nextInt(10) == 0 & (rr.nextInt(4) == 0 || xx != 0 || zz
				// != 0 || yy < 1)) {
		...
		else
		{
			if (this.entityToAttack == null)
			{
				...
				if (rr.nextInt(100) == 0)
				{
					this.antId = this.antId - 50 + rr.nextInt(50);
					this.leader = false;
					this.tracking = null;
				...
				if (this.tracking == null)
				{
					...
					{
						// this.tracking =
						// SQR.getClosestEntityToEntity(this.worldObj, this,
						// 32D, 8);
						if (this.tracking == null)
						{
							this.leader = true;
