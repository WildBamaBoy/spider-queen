...
public final class CrashWatcher extends ModCrashWatcher
{
	...
	protected void onCrash(File crashFile) 
	{
		try
		{
			...
			if (report.contains("at sqr."))
			{
				...
				else
				{
					...
					SQR.getLog().fatal("Detected a crash involving SQR, but crash reporting has been disabled! :(");
		...
		{
			SQR.getLog().fatal("SQR detected a crash and attempted to report it, but failed to do so! " + e.getMessage());
		}
	}
}
