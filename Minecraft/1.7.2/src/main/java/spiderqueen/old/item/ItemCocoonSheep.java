...
public class ItemCocoonSheep extends Item
{

    public ItemCocoonSheep(int i)
    {
        super(i);
        maxStackSize = 1;
    }
    ...
    {
		if(entityplayer.inventory.consumeInventoryItem(mod_SpiderQueen.itemCocoonSheep.shiftedIndex))
        {
            //world.playSoundAtEntity(entityplayer, "random.bow", 1.0F, 1.0F / (itemRand.nextFloat() * 0.4F + 0.8F));
            if(!world.isRemote)
            {
			
					EntityCocoonSheep entCocoon = new EntityCocoonSheep(world, entityplayer.posX, entityplayer.posY, entityplayer.posZ, entityplayer.rotationYaw - 90F,itemstack.getItemDamage());
