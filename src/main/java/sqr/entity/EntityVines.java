...
public class EntityVines extends Entity
{
	...
	public void setVelocity(double d, double d1, double d2)
	{
		...
		if (this.prevRotationPitch == 0.0F && this.prevRotationYaw == 0.0F)
		{
			...
			// prevRotationYaw = rotationYaw = (float)((Math.atan2(d, d2) *
	...
	public void onUpdate()
	{
		...
		if (movingobjectposition != null)
		{
			...
			if (movingobjectposition.entityHit != null)
			{
				...
				if (this.friendly)
				{
					if (movingobjectposition.entityHit instanceof EntityPlayer || movingobjectposition.entityHit instanceof EntitySpiderEx)
					{
						change = true;
					}
					if (movingobjectposition.entityHit instanceof EntityMand)
					{
						...
						if (m.getFriendly() == this.friendly)
						{
							change = true;
						}
					}
				...
				if (!change)
				{
					...
					{
						// worldObj.playSoundAtEntity(this, "random.drr", 1.0F,
			...
			{
				/*
				 * xTile = movingobjectposition.blockX; yTile =
				 * movingobjectposition.blockY; zTile =
				 * movingobjectposition.blockZ; inTile =
				 * worldObj.getBlock(xTile, yTile, zTile); field_28019_h =
				 * worldObj.getBlockMetadata(xTile, yTile, zTile); motionX =
				 * (float)(movingobjectposition.hitVec.xCoord - posX); motionY =
				 * (float)(movingobjectposition.hitVec.yCoord - posY); motionZ =
				 * (float)(movingobjectposition.hitVec.zCoord - posZ); float f1
				 * = MathHelper.sqrt_double(motionX * motionX + motionY *
				 * motionY + motionZ * motionZ); posX -= (motionX / (double)f1)
				 * * 0.05000000074505806D; posY -= (motionY / (double)f1) *
				 * 0.05000000074505806D; posZ -= (motionZ / (double)f1) *
				 * 0.05000000074505806D; worldObj.playSoundAtEntity(this,
				 * "random.drr", 1.0F, 1.2F / (rand.nextFloat() * 0.2F + 0.9F));
				 * inGround = true; arrowShake = 7;
				 */
			}
		}
		...
		// rotationYaw = (float)((Math.atan2(motionX, motionZ) * 180D) /
		// 3.1415927410125732D);
